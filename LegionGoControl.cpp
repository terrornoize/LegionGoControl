...
enum class WorkerJob { Wake, BatteryStatus, BatteryToggle };
...
void StartWorker() {
    { std::lock_guard<std::mutex> lock(g_workerMutex); g_workerStop = false; g_workerJobs.push_back(WorkerJob::BatteryStatus); }
...
void ShowSettingsPage(HWND hwnd, int page) {
    ...
    state->tab = (std::max)(0, (std::min)(2, page));
    TabCtrl_SetCurSel(GetDlgItem(hwnd, IDC_TAB), state->tab);
    for (int index = 0; index < 3; ++index) for (HWND control : state->pages[index]) ShowWindow(control, index == state->tab ? SW_SHOW : SW_HIDE);
}
...
bool ApplySettings(HWND hwnd) {
    ...
    if (!ParseInteger(hwnd, IDC_BASE_STAPM, base.stapm) || !ParseInteger(hwnd, IDC_BASE_FAST, base.fast) || !ParseInteger(hwnd, IDC_BASE_SLOW, base.slow)) {
...
LRESULT CALLBACK SettingsProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        ...
        case WM_NOTIFY: {
            ...
            if (header && header->idFrom == IDC_TAB && header->code == TCN_SELCHANGE) ShowSettingsPage(hwnd, TabCtrl_GetCurSel(header->hwndFrom));
            return 0;
        }
        case WM_COMMAND: {
...
void ShowSettings(int tab) {
    ...
    if (tab == 0) QueueWorker(WorkerJob::BatteryStatus);
...
LRESULT CALLBACK ProfileEditorProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        ...
        case WM_COMMAND: {
            ProfileEditorContext* context = EditorData(hwnd); const int id = LOWORD(wParam); if (!context) return 0;
            if (id == IDC_PE_BROWSE) {
                ...
                if (!ParseInteger(hwnd, IDC_PE_STAPM, candidate.tdp.stapm) || !ParseInteger(hwnd, IDC_PE_FAST, candidate.tdp.fast) || !ParseInteger(hwnd, IDC_PE_SLOW, candidate.tdp.slow)) {
...
void FillProfileList(HWND hwnd) {
    ...
    for (std::size_t index = 0; index < profiles.size(); ++index) {
        ...
        std::wstring stapm = std::to_wstring(tdp.stapm) + L" W", fast = std::to_wstring(tdp.fast) + L" W", slow = std::to_wstring(tdp.slow) + L" W";
        ListView_SetItemText(list, static_cast<int>(index), 2, const_cast<wchar_t*>(stapm.c_str()));
        ListView_SetItemText(list, static_cast<int>(index), 3, const_cast<wchar_t*>(fast.c_str()));
        ListView_SetItemText(list, static_cast<int>(index), 4, const_cast<wchar_t*>(slow.c_str()));
...
void UpdateTrayTip() {
    ...
    tip += L" (" + std::to_wstring(status.desired.stapm) + L"/" + std::to_wstring(status.desired.fast) + L"/" + std::to_wstring(status.desired.slow) + L" W)";
...
void RegisterWindowClass(const wchar_t* name, WNDPROC procedure, HBRUSH background = reinterpret_cast<HBRUSH>(COLOR_WINDOW + 1)) {
    ...
    value.hIcon = LoadIconW(nullptr, IDI_APPLICATION); value.hCursor = LoadCursorW(nullptr, IDC_ARROW); value.hbrBackground = background; RegisterClassW(&value);
}
